 ...
 * @author cpieloth
 */
@Path("/venue/nightlife")
public class RSVenueNightlife extends ARSVenue<JpaEventNightlife> {

    private static final Logger log = LoggerFactory.getLogger(RSVenueNightlife.class);

    private static final JpaEventNightlifeDao eventDAO = JpaEntityDaoFactory.getInstance().getEventNightlifeDao();

    private static final Category cat = Category.NIGHTLIFE;
    ...
    @Override
    protected List<JpaEventNightlife> getEvents(JpaVenue venue, DateHelper.DateRange range) {
    ...
    @Override
    protected Response buildEventResponse(List<JpaEventNightlife> entities) {
        if (entities != null) {
